...
$filter = Join-Path $PSScriptRoot 'render-filter.txt'
...
$output = Join-Path $OutputDir '01-wintools-desktop-lock-douyin.mp4'
...
try {
    ...
    & $ffmpeg -y `
        -f lavfi -i 'color=c=0xF3F0FF:s=1080x1920:r=30:d=22' `
        -i $screen -i $voice `
        -filter_complex_script $filter `
        -map '[v]' -map '2:a:0' -t 22 `
